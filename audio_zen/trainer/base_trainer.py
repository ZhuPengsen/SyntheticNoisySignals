...
class BaseTrainer:
    def __init__(
            self, config, resume, only_validation, model, loss_function, optimizer
    ):
        self.model = model.cuda()
        self.optimizer = optimizer
        self.loss_function = loss_function

        # 大部分情况下，True设置这个flag可以让内置的cuDNN的 auto-tuner自动寻找最适合当前配置的高效算法，来达到优化运行效率的问题。
        ...
        # Backup of project code(暂不确定原因，命令行运行时备份很快，但是在用Pycharm时候很慢，因此暂时屏蔽)
        # shutil.copytree(
    ...
    def _resume_checkpoint(self):
        ...
        print(f"Model checkpoint is loaded. Training will begin at epoch {self.start_epoch}.")
    ...
    def _save_checkpoint(self, epoch, is_best_epoch=False):
        ...
        state_dict = {"epoch": epoch, "best_score": self.best_score, "optimizer": self.optimizer.state_dict(),
                      "scaler": self.scaler.state_dict(), "model": self.model.state_dict()}
        ...
        if is_best_epoch:
            print(f":tada: :tada: :tada: Found a best score in the epoch {epoch}, saving... :thumbsup: :thumbsup: "
                  f":thumbsup: :thumbsup: :thumbsup: :thumbsup:")
    ...
    def spec_audio_visualization(self, noisy, enhanced, clean, name, epoch, mark=""):
        self.writer.add_audio(
            f"{mark}_Speech/{name}_Noisy", noisy, epoch, sample_rate=32000
        )
        self.writer.add_audio(
            f"{mark}_Speech/{name}_Enhanced", enhanced, epoch, sample_rate=32000
        )
        self.writer.add_audio(
            f"{mark}_Speech/{name}_Clean", clean, epoch, sample_rate=32000
        )
    ...
    def metrics_visualization(
        ...
        for metric_name in metrics_list:
            score_on_noisy = [metrics.REGISTERED_METRICS[metric_name](ref, est)
                              for ref, est in zip(clean_list, noisy_list)]
            score_on_enhanced = [metrics.REGISTERED_METRICS[metric_name](ref, est)
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.only_validation is True:
